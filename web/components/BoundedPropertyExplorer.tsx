      ...
      <div className="atlas-workspace">
        ...
        <section className="plot-panel" aria-label="Property plot">
          <div className="plot-title-row">
            <div className="plot-search-bar" role="search"><div className="search-input-shell"><Search size={15}/><input type="search" value={searchQuery} placeholder="Search DOI, authors, title, keyword" onChange={(event) => setSearchQuery(event.target.value)}/>{searchQuery ? <button onClick={() => setSearchQuery("")} aria-label="Clear search"><X size={13}/></button> : null}</div><p className="plot-search-status">{searching ? "Searching…" : searchQuery.trim() ? `${searchResults.length} publications found; ${visibleSearchCount} represented in the active plot.` : "Search publication metadata without changing the active figure filters."}</p></div>
